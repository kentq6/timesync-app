...
export async function updateEvent(
  ...
  unsafeData: z.infer<typeof eventFormSchema> // Raw event data to validate
): Promise<void> {
  try {
    // Authenticate the user using Clerk
    const { userId } = await auth();
...
// This function deletes an existing event in the database after checking ownership
...
// Infer the type if a row rom the EventTable schema
...
export async function getEvent(userId: string, eventId: string): Promise<EventRow | undefined> {
  const event = await db.query.EventTable.findFirst({
    where: ({ id, clerkUserId }, { and, eq }) => 
      and(eq(clerkUserId, userId), eq(id, eventId)), // Make sure the event belonds to the user
  })

  return event ?? undefined; // Explicitly retur undefined if not found
}
...
// "This version of an event is guarenteed to be active - no maybe, no false"

// Async function to fecth all active (public) events for a specific user
