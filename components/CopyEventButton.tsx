...
// VariantProps is a TypeScript utility type from the class-variance-authority (CVA) library.
// It's used to type the props for variants (like size, color, style, etc.)
...
interface CopyEventButtonProps 
  extends Omit<React.ComponentProps<"button">, "children" | "onClick">, // Inherit all native button prop except children and onClick
    VariantProps<typeof buttonVariants> { // Allow variant and size props from button styling
      eventId: string // Required: event ID for the bookling link
...
export function CopyEventButton({
  ...
  return (
    ...
    >
      ...
      {getCopyLabel(copyState)} {/* Tect label that changes with copy state */}
    </Button>
  )
}