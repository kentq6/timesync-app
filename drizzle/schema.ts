...
export const EventTable = pgTable(
  "events", // table name in the database
  {
    ...
    // .primaryKey(): Makes the UUID the primary key of the table
    ...
    clerkUserId: text("clerk").notNull(), // ID of the user who created it (from Clerk)
...
export const ScheduleAvailabilityTable = pgTable(
  "scheduleAvailabilities", 
  {
    ...
    scheduleId: uuid("scheduleId") // foreign key to the Schedule table
      .notNull().
      references(() => ScheduleTable.id, { onDelete: "cascade" }), // cascase delete when schedule is deleted
    startTime: text("startTime").notNull(), // start time of availability (e.g., "09:00")
    endTime: text("endTime").notNull(), // end time of availiability (e.g., "17:00")
